...
def process_transactions_topic(spark):
    """Traitement du topic transactions avec gestion des offsets"""
...
def process_json_topic(spark, topic_name, destination_bucket):
    ...
    # Afficher les offsets traités
    print(f"=== OFFSETS TRAITÉS POUR {topic_name.upper()} ===")
    json_df.select("kafka_partition", "kafka_offset").show()
...
def main():
    """Fonction principale pour traiter tous les topics avec gestion des offsets"""
    print("=== DÉMARRAGE DU JOB KAFKA VERS MINIO MULTI-TOPICS (MODE INCREMENTAL) ===")
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print("=== TOUS LES TOPICS TRAITÉS (MODE INCREMENTAL) ===")
        print(f"{'='*60}")
