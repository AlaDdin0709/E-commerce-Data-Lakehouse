...
def create_spark_session():
    ...
    spark = SparkSession.builder \
        .appName("CustomerImagesKafkaProcessor") \
...
def process_customer_images_data(spark):
    """Traiter les données du topic customer_images (JSON)"""
    print("=== TRAITEMENT DU TOPIC CUSTOMER IMAGES ===")
    
    bootstrap_servers = "172.17.34.127:9092"
    starting_offsets = read_last_offsets(spark, "customer_images", bootstrap_servers)
    
    df = spark.read \
        .format("kafka") \
        .option("kafka.bootstrap.servers", bootstrap_servers) \
        .option("subscribe", "customer_images") \
    ...
    if df.count() == 0:
        print("Aucun nouveau message trouvé dans Kafka pour customer_images.")
        return None
    ...
    print("=== EXEMPLES DE DONNÉES JSON CUSTOMER IMAGES ===")
    json_df.select("json_data").show(3, truncate=False)
    
    return json_df

def write_customer_images_to_json(df, output_path):
    """Écrire les données customer images vers MinIO en format JSON"""
    
    if df is None or df.count() == 0:
        print("Aucune donnée customer images à écrire dans MinIO")
        return
    
    print(f"Écriture de {df.count()} enregistrements customer images vers MinIO...")
    ...
    try:
        ...
        print(f"Données customer images écrites avec succès dans: {output_path}")
        
        print("=== STATISTIQUES DES DONNÉES CUSTOMER IMAGES ÉCRITES ===")
        ...
        if "FileAlreadyExistsException" in str(e) or "destination file exists" in str(e):
            ...
            try:
                ...
                print(f"✅ Données customer images écrites avec timestamp unique: {unique_path}")
            ...
            try:
                ...
                print("✅ Données customer images écrites avec timestamp unique!")
            except Exception as e2:
                print(f"❌ Erreur lors de l'écriture avec timestamp: {str(e2)}")
                raise e2

def main():
    """Fonction principale pour traiter le topic customer_images"""
    print("=== DÉMARRAGE DU TRAITEMENT CUSTOMER IMAGES ===")
    ...
    try:
        ...
        processed_df = process_customer_images_data(spark)
        
        if processed_df is not None and processed_df.count() > 0:
            output_path = "s3a://customer-images/raw"
            write_customer_images_to_json(processed_df, output_path)
            
            save_last_offsets(spark, processed_df, "customer_images")
            
            print(f"✅ Topic customer_images traité avec succès! {processed_df.count()} enregistrements traités.")
        else:
            print("⚠️ Aucune donnée valide à traiter pour customer_images")
            
    except Exception as e:
        print(f"❌ Erreur lors du traitement de customer_images: {str(e)}")
        import traceback
        traceback.print_exc()
        raise e
    finally:
